...
The adapters will consist of a client (for open ai api calls), repository (for loading and saving files, eg. getImageNames, loadImageByName, saveOutput), and command (please use bubble tea to collect config params). Each adapter should include test cases for the happy path and distinct error cases that are defined in the package. Errors related to an adapter implementation should be defined as variables in the adapter that returns them for easy testing. For api errors, please use an api error struct that contains the status and any error message returned by the server.

## Tasks
Use tasks.md as you task manager. This project may already be in progress. You will find your next task there. There are instructions found there about how to manage, define and complete the tasks in the list. 

## Notes
User notes.md for notes that help you carry context from task to task. You're freed to add additional guidelines that are provided during the review process there as a form of working memory. 
